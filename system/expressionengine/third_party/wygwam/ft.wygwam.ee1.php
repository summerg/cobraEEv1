...
class Wygwam extends Fieldframe_Fieldtype {
	...
	private function _theme_url()
	{
		if (! isset($this->_theme_url))
		{
			...
			$this->_theme_url = $theme_folder_url.'wygwam/';
		}
	...
	private function _config_json($settings)
	{
		...
		if (! isset($config['language']))
		{
			...
			$config['language'] = isset($this->helper->lang_map[$language]) ? $this->helper->lang_map[$language] : 'en';
		}
	 ...
	 * Hidden Input
	 */
	private function _hidden_input($field_name, $data)
	{
		return '<div style="display: none;">'
		       . '<input type="hidden" name="'.$field_name.'[old]" value="'.str_replace(array('"', '&'), array('&quot;', '&amp;'), $data).'" />'
		       . '</div>';
	}
	...
	function display_field($field_name, $data, $settings)
	{
		...
		return $this->_hidden_input($field_name, $data)
		       . '<div style="margin:0 8px 15px 0;"><textarea id="'.$id.'" name="'.$field_name.'[new]">'.$data.'</textarea></div>';
	}
	...
	function display_cell($cell_name, $data, $settings)
	{
		...
		return $this->_hidden_input($cell_name, $data)
		       . '<textarea id="'.$id.'" name="'.$cell_name.'[new]">'.$data.'</textarea>';
	}
	...
	{
		$data = isset($data['new']) ? $data['new'] : $data['old'];
